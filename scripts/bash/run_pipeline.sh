...
stage "3" "DBT SILVER" bash -c 'cd walmart_dbt && uv run dbt run --select silver && uv run dbt test --select silver'
stage "4" "SILVER SQL TESTS" uv run python scripts/python/sql_test.py tests/silver
stage "5" "DBT GOLD" bash -c 'cd walmart_dbt && uv run dbt run --select gold && uv run dbt test --select gold'
